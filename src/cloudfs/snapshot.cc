...
SnapshotController::SnapshotController(
  ...
  // logger_->debug(
  //     "SnapshotController::SnapshotController: .snapshot info exists, size: " +
...
int SnapshotController::create_snapshot(unsigned long *timestamp) {
  ...
  // logger_->debug(
  //     "SnapshotController::create_snapshot: snapshot chunk table done");
...
int SnapshotController::restore_snapshot(unsigned long *timestamp) {
  ...
  // logger_->debug(
  //     "SnapshotController::restore_snapshot: restore chunk table done");
...
int SnapshotController::delete_snapshot(unsigned long *timestamp) {
  ...
  cloudfs_controller_->get_chunk_table().DeleteSnapshot(
      tmp_file); // chunk table will handle decreasing snapshot_ref_count
...
int SnapshotController::install_snapshot(unsigned long *timestamp) {
  ...
  cloudfs_controller_->get_chunk_table().SkipSnapshot(tmp_file); // skip chunk table
