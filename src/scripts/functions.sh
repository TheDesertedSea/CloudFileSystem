...
assert_endpoint_active() {
  curl -o /dev/null -s -f "$1" || {
    echo "S3 server not running?"
    exit 1
  }
}

assert_endpoint_inactive() {
  curl -o /dev/null -s -f "$1" && {
    echo "S3 server is already running. Please kill manually. "
    exit 1
  } || true
}
...
function reinit_env()
{
  ...
  local endpoint="localhost:8888"
  assert_endpoint_inactive $endpoint

  nohup python3 $SCRIPTS_DIR/../s3-server/s3server.pyc > /dev/null 2>&1 &
  if [ $? -ne 0 ]; then
  ...
  assert_endpoint_active $endpoint
