...
class BaseRequestHandler(web.RequestHandler):
    ...
    def render_xml(self, value):
        ...
        parts = []
        parts.append(
            escape.utf8(f'<{name} xmlns="http://doc.s3.amazonaws.com/2006-03-01">')
        )
        self._render_parts(list(value.values())[0], parts)
        parts.append(escape.utf8(f"</{name}>"))
        for p in parts:
            tmon.num_read_bytes += len(p)

        self.finish(b'<?xml version="1.0" encoding="UTF-8"?>\n' + b"".join(parts))

    def _render_parts(self, value, parts=[]):
        if isinstance(value, (str, bytes)):
            parts.append(escape.xhtml_escape(value).encode("utf-8"))
        elif isinstance(value, int):
            parts.append(str(value).encode("utf-8"))
        elif isinstance(value, datetime.datetime):
            parts.append(value.strftime("%Y-%m-%dT%H:%M:%S.000Z").encode("utf-8"))
...
class ObjectHandler(BaseRequestHandler):
    def get(self, bucket, object_name):
        ...
        object_name = escape.url_unescape(object_name)
...
def main():
    try:
        (opts, args) = getopt.getopt(
            sys.argv[1:],
            "p:h:t:l:v",
            ["port=", "host=", "target=", "log=", "verbose", "help"],
        )
    ...
    try:
        p = subprocess.Popen(["du", "-c", target], stdout=subprocess.PIPE)
