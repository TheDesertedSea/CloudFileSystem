...
CloudfsController::CloudfsController(struct cloudfs_state *state, const std::string& host_name, std::string bucket_name, std::shared_ptr<DebugLogger> logger) 
  : state_(state), bucket_name_(std::move(bucket_name)), logger_(std::move(logger)), buffer_controller_(std::make_shared<BufferFileController>(host_name, bucket_name_, logger_)) {
...
int CloudfsControllerDedup::write_file(const std::string& path, uint64_t fd, const char* buf, size_t size, off_t write_offset) {
  ...
  for(int i = rechunk_start_idx; i <= (buffer_end_idx == -1 ? (int)chunks.size() - 1 : buffer_end_idx); i++) {
    auto& chunk = chunks[i];
    logger_->info("write_file: old chunk start " + std::to_string(chunk.start_) + ", len " + std::to_string(chunk.len_) + ", key " + chunk.key_);
  }
  ...
  for(auto& c: new_chunks) {
    logger_->info("write_file: new chunk start " + std::to_string(c.start_) + ", len " + std::to_string(c.len_) + ", key " + c.key_);
  }
